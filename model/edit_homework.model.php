	...
	public function getHomework($id) {
		...
		if (isset($_POST['btn-update']) && $_SERVER['REQUEST_METHOD'] == 'POST') {
			...
			if (empty($error_title) && empty($error_video_type) && empty($error_video_size) && empty($error_image_type) && empty($error_image_size) && empty($error_audio_type) && empty($error_audio_size) && empty($error_pdf_type) && empty($error_pdf_size)) {
				...
				$sql = "UPDATE homeworks SET title = :title, date_hw = :date_hw, commenter = :commenter, link = :link, video = :video, img = :img, audio = :audio, pdf = :pdf, grado = :grado, grupo = :grupo WHERE id = :id";
